...
'''
    通过将每个重复测点的数据分80%训练模型，另20%验证模型；然后将另重复测点数据的20%交互进行模型评价，
    选取准确度最高的测点数据集

'''


class detectorSelect(n_detectors,self):
    def transform():        # 将单测点转换成三列数据
        self_n = pd.concat(self.index,self_index.iloc[:,0])
        return self_n
